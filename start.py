from schedule import every, run_pending, get_jobs, clear
...
def schedule_tasks():
    ...
    if timetable:
        ...
        for day_schedule in timetable:
            ...
            getattr(every(), day_name).at(checkin_hour).do(
                random_checkin).tag("check", "checkin-tasks")
            getattr(every(), day_name).at(checkout_hour).do(
                random_checkout).tag("check", "checkout-tasks")
            LOGGER.info(
                f"{day_name.capitalize()} scheduled: check-in at {checkin_hour} and check-out at {checkout_hour}")
...
def main():
    if INTERNAL_SCHEDULING:
        ...
        getattr(every(), timetable_update_weekday).at(
            timetable_update_time).do(schedule_tasks).tag("schedule")
        LOGGER.info(
            f"Timetable updating task enabled for {timetable_update_weekday.capitalize()}s at {timetable_update_time}")
        schedule_tasks()
        while True:
            run_pending()
            sleep(1)
